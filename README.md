...
| Free-running, post-trim, **at calibration point** (T = 27 °C, process, VDD ±10%) | **−9.2% / +6.5%** worst-case | quantization ±0.157% + trim-DAC mismatch ±0.300% + comparator offset ±0.300% (both still flagged assumptions) + the **measured** supply span at per-corner calibrated codes (−8.4%/+5.8% worst across both summary bases, of which 62–82% is the comparator/latch delay residue and 16–36% a charge-path supply term, per the delay-probe campaign `sim/pvt/results/20260922T004823Z/`) — DR-0015 supersedes DR-0003's ±1.1%, whose ±0.300% supply line assumed away the delay path DR-0003 never budgeted. The figure is now ~3× looser than ST's shipped `ACC_HSI48` = −2.8/+2.9% at 25 °C — the measured cost of an unregulated asynchronous delay path on this node; circuit-side tightening lever filed as #57 |
| Free-running, post-trim, **full temperature range** (−40…+85 °C, VDD ±10%) | **+13.4% / −17.1%** worst-case | gf180mcu's published poly-resistor TCR of −1200 ppm/K (§6.1A), trimmed at 27 °C: ΔR/R = −6.96% at +85 °C (58 K) so f rises +6.84%, ΔR/R = +8.04% at −40 °C (67 K) so f falls −7.91%, plus the re-derived at-calibration figure above — DR-0015 (supersedes DR-0003's +8%/−9% by the same linear worst-case sum; DR-0004 stands — no active TC compensation, the delay term is budgeted, not compensated — and closing the gap to USB compliance remains entirely the job of the runtime-disciplined row below) |
