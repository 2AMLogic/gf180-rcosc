...
| Supply sensitivity, slow-corner calibrated codes (`ss`/`sf`/`rc_s`) | ~0 (ratiometric ideal) | +20.05% / +12.02% / +13.70% | **+11.76% / +7.73% / +6.09%** | roughly halved — the phantom share removed; the remainder is re-attributed to the comparator/latch delay residue by the campaign's own per-code rows (DR-0013) |
