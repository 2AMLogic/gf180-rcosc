#!/usr/bin/env python3
"""gf180-rcosc quiescent-current (Iq) sweep driver — issue #22, extended #24.
...
Issue #24 / DR-0009: the `as-committed` sizing is now re-derived against
...
to agree on verdict at every sizing from this point on -- DR-0008's
committed sizing happened to satisfy both, but that was a consequence of
sizing against the stricter `.op` figure, not a general property. Report
prose below states each metric's verdict without asserting agreement.
...
def write_results(runid, rows, results_dir, ngspice_ver, pdk_root, git_sha, dirty):
    ...
    lines = [
        f"# Quiescent current (Iq) sweep {runid} — issue #22 / #24",
        "",
        "Generated by `sim/iq/iq_sweep.py` (issue #22, extended #24)."
