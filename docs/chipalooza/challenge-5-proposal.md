...
| 9 | Quiescent current, running (`< 500 µA`) | < 500 µA (running) | **Met at every simulated code**, worst case **468.61 µA** at code `0xFF` (`iq_run`, the metric this row actually names, per [DR-0008](../../spec/decision-records/0008-iq-metric-correction-and-bias-rebalance.md)/[DR-0009](../../spec/decision-records/0009-running-iq-metric-basis-and-partial-trim-range-recovery.md)); the DC-equilibrium `.op` figure (`iq_op`), reported for continuity only and **not** this row's verdict basis, is 754.90 µA at the same code — reported here so neither metric is quoted alone | `sim/iq/results/20260907T090639Z/README.md` | **Met** (on the row's actual basis, `iq_run`) |
...
[DR-0009](../../spec/decision-records/0009-running-iq-metric-basis-and-partial-trim-range-recovery.md)
sizing) except row 9, which cites `sim/iq/results/20260907T090639Z/`
(the matching Iq sweep at the same sizing).
